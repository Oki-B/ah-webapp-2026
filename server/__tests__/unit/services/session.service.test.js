...
describe("SessionService Unit Test", () => {
  ...
  describe("createNewSession()", () => {
    it("should revoke oldest session if active sessions count >= 5", async () => {
      ...
      userSessionRepository.createSession.mockResolvedValue({ id: "new-session-id" });
      generateTokenPair.mockReturnValue({ rawToken: "raw", hashedToken: "hashed" });
      ...
      expect(userSessionRepository.revokeOldestSession).toHaveBeenCalledWith(mockUserId, { transaction: mockT });
  ...
  describe("refreshSession()", () => {
    ...
    it("should throw 401 if session is not found (Invalid Token)", async () => {
      ...
      await expect(sessionService.refreshSession({ 
        oldRefreshToken: oldRawToken, 
        deviceInfo: mockDeviceInfo 
      })).rejects.toThrow("Invalid refresh token");
    });

    it("should throw 401 and revoke if token is reused/expired", async () => {
      const expiredSession = { 
        id: "session-123", 
        expiresAt: new Date(Date.now() - 1000) // Sudah lewat
      };
      userSessionRepository.findValidSessionByToken.mockResolvedValue(expiredSession);
      ...
      // Verifikasi: Session ID tersebut langsung di-revoke demi keamanan
      expect(userSessionRepository.revokeSessionById).toHaveBeenCalledWith(expiredSession.id, expect.any(Object));
    });
    ...
    it("should successfully rotate token for valid session", async () => {
      ...
      // Mock sequence
      userSessionRepository.findValidSessionByToken.mockResolvedValue(validSession);
      userSessionRepository.countActiveSessions.mockResolvedValue(1);
      userSessionRepository.createSession.mockResolvedValue({ id: "new-session-id" });
      generateTokenPair.mockReturnValue({ rawToken: "new-raw", hashedToken: "new-hashed" });
      ...
      expect(userSessionRepository.revokeSessionById).toHaveBeenCalledWith(validSession.id, expect.any(Object));
    });
  });
  ...
  describe("revokeFromDevice()", () => {
    it("should throw 404 if session doesn't belong to user", async () => {
      ...
      await expect(sessionService.revokeFromDevice(mockUserId, "sid"))
    ...
    it("should successfully revoke session", async () => {
      ...
      expect(userSessionRepository.revokeSessionById).toHaveBeenCalledWith("sid", expect.any(Object));
    });
  });
});